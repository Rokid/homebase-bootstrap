...
function getToken () {
  function doCheck() {
    ...
    var url = `https://apigwrest${env}.open.rokid.com/v1/sts/StsService/GetToken`
    console.log('=====url=====', url)
    return rp({
      uri: url,
      method: 'POST',
  ...
  doCheck().then(result => {
    ...
    if (!stsInfo
        ...
        || !stsInfo.expiration
        || !stsInfo.token) {
      console.error(`get token error ${JSON.stringify(stsInfo)}`)
    ...
    console.log(`refreshTokenTime=${refreshTokenTime}, request result=${JSON.stringify(stsInfo)}`)
  })
  .catch(err => {
