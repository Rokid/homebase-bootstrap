var rp = require('@rokid/httpdns')().requestWithDNS
var catchLog = require('./lib/catchLog').getInstance()
...
function getToken () {
  function doCheck() {
    ...
    var env = profile.env === 'dev' || profile.env === 'test' ? '-ctyun': ''
    var url = `https://cloudapigw${env}.open.rokid.com/v1/sts/StsService/GetToken`
    uploadLog(`[uploadLog] url ${url}`)
    return rp({
      uri: url,
      method: 'POST',
      timeout: 15 * 1000,
    ...
    .catch(err => {
      uploadLog(`[uploadLog] error ${env} ${props} ${authKey}` )
      throw err
    })
  }
  doCheck().then(result => {
    var stsInfo = result && result.stsInfo
    if (!stsInfo
        || result.message !== 'success'
        || result.resultCode === 1
        || typeof stsInfo !== 'object' 
        || !stsInfo.accessKeyId
        || !stsInfo.accessKeySecret
        || !stsInfo.expiration
        || !stsInfo.token) {
      uploadLog(`[uploadLog] get token error ${JSON.stringify(stsInfo)}`)
      setTimeout(getToken, getTokenFailureInterval)
      return
    }
    var expiration = new Date(stsInfo.expiration).getTime()
    var refreshTokenTime = expiration - Date.now() - 5 * 60 * 1000
    if (refreshTokenTime <= 0) refreshTokenTime = getTokenFailureInterval
    ...
    uploadLog(`[uploadLog] refreshTokenTime: ${refreshTokenTime} ms, expiration: ${stsInfo.expiration}`)
  })
  .catch(err => {
      uploadLog('[uploadLog] update error', err)
      setTimeout(getToken, getTokenFailureInterval)
  })
}
...
Logger.prototype._doLog = function (level, args, prefix, suffix, pid) {
  if (!enableUpload && !enableStdPrint && !pipeToParent) {
    return
  }
  var now = new Date(new Date().getTime() + 28800000).toJSON()
  ...
  if (fixBootCatch) {
    catchLog.saveLog(now, level, msg, this.name, pid || process.pid)
  }
...
function uploadLog () {
  Logger.prototype._doLog.call({ name: 'uplo' }, 'info', arguments, '\x1B[0;33m', '\x1B[0m')
}

Logger.prototype.readLogStream = function (num) {
  return catchLog.readLogStream(num)
}

Logger.prototype.readStream = function (filePath, options) {
  return catchLog.readStream(filePath, options)
}
...
module.exports = {
   ...
   * @param {Boolean} options.fixBootCatch 针对 boot版本 >= '2.0.1' <= '2.2.6'
   *  默认开启本地缓存日志 默认false
  ...
  initGlobalOptions: function (options) {
    ...
    this.setFixBootCatch(options.fixBootCatch)
  ...
  setFixBootCatch: function (b) {
    fixBootCatch = typeof b === 'boolean' ? b : false
  },
